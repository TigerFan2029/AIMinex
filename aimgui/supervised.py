...
class supervised_learning():
    ...
    def graph_icon(self):
        ...
        image_path = 'images/icons/ml.png'
        ...
        ToolTip(self.image_button, msg="Supervised Learning-Data Prediction")
    ...
    def initialize_ui(self):
        ...
        self.import_button.grid(row=6, column=0, columnspan=4, pady=10, sticky="ew")
        ...
        self.combo1.grid(row=0, column=0, columnspan=4, padx=10, pady=5, sticky="ew")
        ...
        self.plus_button.grid(row=7, column=0, columnspan=4, pady=10, sticky="ew")
        ...
        self.predict_button.grid(row=8, column=0, columnspan=4, pady=10, sticky="ew")
    ...
    def create_y_column_combobox(self):
        ...
        ctk.CTkLabel(self.scrollable_frame, text="Predict for:").grid(row=5, column=0, padx=10, pady=5, sticky="ew")
        ...
        self.y_column_combo.set("Select Column")
        self.y_column_combo.grid(row=5, column=1, columnspan=3, padx=10, pady=5, sticky="ew")
    ...
    def update_model_parameters(self, selected_model):
        ...
        if selected_model == "SVM":
            ...
            self.kernel_text = ctk.CTkLabel(self.scrollable_frame, text="Select Kernel")
            self.kernel_text.grid(row=param_row_start, column=1, padx=10, pady=5, sticky="w")
            self.kernel_combo = ctk.CTkComboBox(self.scrollable_frame, values=["linear", "poly", "rbf", "sigmoid"])
            self.kernel_combo.grid(row=param_row_start, column=2, columnspan=2, padx=10, pady=5, sticky="ew")
            self.kernel_combo.set("linear")
            
            self.c_text = ctk.CTkLabel(self.scrollable_frame, text="Select C")
            self.c_text.grid(row=param_row_start + 1, column=1, padx=10, pady=5, sticky="w")
            self.c_spinbox = ttk.Spinbox(self.scrollable_frame, width=15, from_=0.1, to=1000, increment=0.1)
            self.c_spinbox.grid(row=param_row_start + 1, column=2, columnspan=2, padx=10, pady=5, sticky="ew")
            self.c_spinbox.set("1.0")

            self.coef0_text = ctk.CTkLabel(self.scrollable_frame, text="Select Coef0")
            self.coef0_text.grid(row=param_row_start + 2, column=1, padx=10, pady=5, sticky="w")
            self.coef0_spinbox = ttk.Spinbox(self.scrollable_frame, width=15, from_=0.0, to=10, increment=0.1)
            self.coef0_spinbox.grid(row=param_row_start + 2, column=2, columnspan=2, padx=10, pady=5, sticky="ew")
            self.coef0_spinbox.set("0.0")
            
            self.degree_text = ctk.CTkLabel(self.scrollable_frame, text="Select Degree")
            self.degree_text.grid(row=param_row_start + 3, column=1, padx=10, pady=5, sticky="w")
            self.degree_spinbox = ttk.Spinbox(self.scrollable_frame, width=15, from_=1.0, to=10)
            self.degree_spinbox.grid(row=param_row_start + 3, column=2, columnspan=2, padx=10, pady=5, sticky="ew")
            self.degree_spinbox.set("3")

        elif selected_model == "Random Forest":
            # Add Random Forest parameter widgets starting at row 2
            self.n_estimators_text = ctk.CTkLabel(self.scrollable_frame, text="Select n_estimators")
            self.n_estimators_text.grid(row=param_row_start, column=1, padx=10, pady=5, sticky="w")
            self.n_estimators_spinbox = ttk.Spinbox(self.scrollable_frame, width=15, from_=10.0, to=1000)
            self.n_estimators_spinbox.grid(row=param_row_start, column=2, columnspan=2, padx=10, pady=5, sticky="ew")
            self.n_estimators_spinbox.set("100")

            self.max_depth_text = ctk.CTkLabel(self.scrollable_frame, text="Select max_depth")
            self.max_depth_text.grid(row=param_row_start + 1, column=1, padx=10, pady=5, sticky="w")
            self.max_depth_spinbox = ttk.Spinbox(self.scrollable_frame, width=15, from_=1.0, to=100)
            self.max_depth_spinbox.grid(row=param_row_start + 1, column=2, columnspan=2, padx=10, pady=5, sticky="ew")
            self.max_depth_spinbox.set("None")

            self.min_samples_split_text = ctk.CTkLabel(self.scrollable_frame, text="Select min_samples_split")
            self.min_samples_split_text.grid(row=param_row_start + 2, column=1, padx=10, pady=5, sticky="w")
            self.min_samples_split_spinbox = ttk.Spinbox(self.scrollable_frame, width=15, from_=2.0, to=10)
            self.min_samples_split_spinbox.grid(row=param_row_start + 2, column=2, columnspan=2, padx=10, pady=5, sticky="ew")
    ...
    def add_manual_row(self, column_name=None, values=None):
        ...
        if len(self.rows) < len(self.df.columns):
            ...
            spinbox = ttk.Spinbox(self.scrollable_frame, width=15, from_=0.0, to=100, increment=0.1)
            ...
            delete_button.grid(row=current_row, column=2, padx=10, pady=5, sticky="w")

            self.rows.append([combobox, spinbox, delete_button])

            self.predict_button.grid(row=current_row + 1, column=0, columnspan=4, pady=10, sticky="ew")
        ...
        if row_index is not None:
            ...
            for idx, row in enumerate(self.rows):
                for col_idx, widget in enumerate(row):
                    widget.grid(row=idx + 8, column=col_idx, padx=10, pady=5, sticky="w")
