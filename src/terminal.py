...
class MATerminal:
    ...
    def get_scope(self):
        ...
        scope['color_change'] = color_change
        return scope
    ...
    def register_to_scope(self, name, obj):
        # Add the new object to the interpreter's locals (scope)
    ...
    def enter_command(self, event):
        ...
        # If the command is just empty (or after removing the prompt it becomes empty), just show a new prompt
        if not command.strip():
            self.terminal_text.insert(tk.END, "\n")  # Just insert a newline if the command is empty
            self.prompt()  # Show the prompt again
            return "break"
        
        # If the command is non-empty, proceed as normal
        self.terminal_text.insert(tk.END, "\n")
